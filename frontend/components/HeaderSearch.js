      ...
      <div className={'overflow-auto'}   >
       ...
       <div className={`z-50`}>
            ...
            : (search && filterCoins.length>0 ?
              <ul  className={`grid   2xl:grid-cols-1 xl:grid-cols-1 grid-cols-1  m-auto z-50 overflow-y-scroll slim-scroll container-line `+ (filterCoins.length>=8 ? 'h-500' : "") }>
              {(filterCoins.map((card)=>(<li key={card.ticker}  onMouseDown={()=>{ router.push({pathname:`/detail/`, query:{asset:card.ticker}})}}><MiniCard key={card.ticker} asset={card}></MiniCard></li>)))}
              </ul>
